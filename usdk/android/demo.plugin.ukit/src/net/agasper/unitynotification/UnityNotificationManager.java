...
public class UnityNotificationManager extends BroadcastReceiver
{
    ...
    @SuppressLint("NewApi")
	public static void CreateChannel(Activity activity,String identifier, String name, String description, int importance, String soundName, int enableLights, int lightColor, int enableVibration, long[] vibrationPattern, String bundle) {
    ...
    @TargetApi(24)
    private static void createChannelIfNeeded(Activity activity, String identifier, String name, String soundName, boolean enableLights, boolean enableVibration, String bundle) {
        if (channels.contains(identifier))
            return;
        channels.add(identifier);

        CreateChannel(activity,identifier, name, identifier + " notifications", NotificationManager.IMPORTANCE_DEFAULT, soundName, enableLights ? 1 : 0, Color.GREEN, enableVibration ? 1 : 0, null, bundle);
    }

    public static void SetNotification(Activity activity, int id, long delayMs, String title, String message, String ticker, int sound, String soundName, int vibrate,
                                       ...
                                       ArrayList<NotificationAction> actions)
    {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            if (channel == null)
                channel = "default";
            createChannelIfNeeded(activity,channel, title, soundName, lights == 1, vibrate == 1, bundle);
        }
    ...
    public static void SetRepeatingNotification(Activity activity,int id, long delayMs, String title, String message, String ticker, long rep, int sound, String soundName, int vibrate, int lights,
    ...
    public static void CancelPendingNotification(Activity activity,int id)
    {
        Activity currentActivity = activity;
        AlarmManager am = (AlarmManager)currentActivity.getSystemService(Context.ALARM_SERVICE);
        Intent intent = new Intent(currentActivity, UnityNotificationManager.class);
