...
namespace UnityEditor.iOS.Xcode
{
    public class PBXProject
    {
        ...
        {
            string target = TargetGuidByName("UnityFramework");
            if (target == null)
                target = TargetGuidByName("Unity-iPhone");
            return target;
        }
        ...
        private void AddBuildFileImpl(string targetGuid, string fileGuid, bool weak, string compileFlags)
        {
            ...
            string extension = Path.GetExtension(fileReferenceData.path);
            if (!FileTypeUtils.IsBuildable(extension, fileReferenceData.isFolderReference) || this.BuildFilesGetForSourceFile(targetGuid, fileGuid) != null)
                return;
            ...
            this.BuildSectionAny(target, extension, fileReferenceData.isFolderReference).files.AddGUID(fromFile.guid);
        }
